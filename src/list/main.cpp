...
{  //创建长度为n的列表，其元素随机取自[0, 4n)
    ListNodePosi<T> p = (rand() % 2) ? list.insertAsLast(rand() % (T)(n * 4)) : list.insertAsFirst(rand() % (T)(n * 4));
    for (Rank i = 1; i < n; i++)
        p = rand() % 2 ? list.insert(rand() % (T)(n * 4), p) : list.insert(p, rand() % (T)(n * 4));
}
...
void testList(Rank testSize)
{
    ...
    List<T> Lx;
    randomList(Lx, testSize);
    PRINT(Lx);
    Lx.sort();
    PRINT(Lx);
    List<T> Ly;
    randomList(Ly, testSize);
    PRINT(Ly);
    Ly.sort();
    PRINT(Ly);
    ...
    while (0 < --i)
    {
        La[i - 1]->data += La[i]->data;
        La[i - 1]->data >>= 1;
    }
    PRINT(La);  // call-by-rank, clear but slow
    printf("\n  ==== Test %2d. reverse\n", testID++);
    La.reverse();
    PRINT(La);
    printf("\n  ==== Test %2d. Copy all\n", testID++);
    ...
    printf("\n  ==== Test %2d. FIND in\n", testID++);
    PRINT(Lf);
    for (Rank i = 0; i <= testSize * 2; i++)
    {  //逐一测试[0, 2n]中的所有可能
        ListNodePosi<T> p = Lf.find((T)i);
        printf("Looking for ");
        print((T)i);
        printf(": ");
        if (p)
